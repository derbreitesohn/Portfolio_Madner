...
import { primaryLink, projectsData } from "@/lib/data";
    ...
    <section id="projects" className="portfolio-section" aria-labelledby="work-title">
      ...
      <div className="portfolio-project-grid">
        {selectedProjects.map((project, index) => {
          const cover = primaryLink(project);
          return (
            <article key={project.slug} className="portfolio-project" aria-labelledby={`project-${project.slug}`}>
              <a href={cover.href} target="_blank" rel="noopener noreferrer" className={`portfolio-project-image portfolio-project-image-${project.slug}`} aria-label={`${project.displayTitle}: ${cover.label.toLowerCase()} (opens in a new tab)`}>
              ...
              <div className="portfolio-project-links">
                {project.demo && (
                  <a href={project.demo.href} target="_blank" rel="noopener noreferrer" className="portfolio-text-link" aria-label={`${project.demo.label}: ${project.displayTitle} (opens in a new tab)`}>
                    {project.demo.label}
                  </a>
                )}
                <a href={project.source.href} target="_blank" rel="noopener noreferrer" className="portfolio-text-link" aria-label={`${project.source.label}: ${project.displayTitle} on GitHub (opens in a new tab)`}>
