...
- The gallery and waterside step keep the artist's `MAT_Base_WeatheredStone` dirt/stone bakes and original UVs. `lib/museum/floor.ts` sets normal strength to 0.3 and matte roughness to 0.92. The original 800-pixel normal map remains connected to UV0, with its repeat calculated from each level's UV scale so the stones are approximately half a metre wide. The gallery dirt/colour atlas retains its full 2048 resolution on UV1. No replacement texture pixels, extra floor geometry or collision changes.
